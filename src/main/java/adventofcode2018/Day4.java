...
public class Day4 {
	...
	private static class Record implements Comparable<Record> {
		...
		public Record(String line) {
			...
			try {
				String dateString = split[0].replace("[", "");
	...
	public static void main(String[] args) {
		...
		Map<String, int[]> guards = new HashMap<>();
		String[] strings = input.split("\\n");
		List<Record> allRecords = new ArrayList<>();

		for (int i = 0; i < strings.length; i++) {
			Record r = new Record(strings[i]);
			allRecords.add(r);
		}
		...
		for (Record r : allRecords) {
			if (r.payload.startsWith("Guard")) {
				String id = r.payload.split(" ")[1].replace("#", "");
				if (!guards.containsKey(id)) {
					minutes = new int[60];
					Arrays.fill(minutes, 0);
					guards.put(id, minutes);
				} else {
					minutes = guards.get(id);
				}
			} else if (r.payload.equals("falls asleep")) {
				minuteFallsAsleep = r.date.getMinutes();
			} else { // wakes up
				for (int i = minuteFallsAsleep; i < r.date.getMinutes(); i++) {
					minutes[i]++;
				}
			}
		}
		;
		// guards.forEach((k, v) -> System.out.println(k + " -> sum= " +
		// Arrays.stream(v).sum() + " : " + Arrays.toString(v)));
		String maxId = "";
		int absolutMaxValue = 0;
		for (Map.Entry<String, int[]> entry : guards.entrySet()) {
			String key = entry.getKey();
			int[] value = entry.getValue();
			// int max = Arrays.stream(value).sum();
			int max = Ints.max(value);
			if (max > absolutMaxValue) {
				absolutMaxValue = max;
				maxId = key;
			}
		}

		int[] is = guards.get(maxId);
		for (int i = 0; i < is.length; i++) {
			if (absolutMaxValue == is[i]) {
				System.out.println("maxId: " + maxId + " maxvalue:" + absolutMaxValue);
				System.out.println(Arrays.toString(is));
				System.out.println(Integer.parseInt(maxId) * i);

			}
		}
	}

	private final static String input = "[1518-10-14 00:05] falls asleep\n" + "[1518-09-13 00:12] falls asleep\n"
